	...
	int height)
{
	...
	//·»´õ Å¸°Ù ºä »ý¼º
	result = device->CreateRenderTargetView(renderTargetTexture, &rtvDesc, &renderTargetView);
	...
	//SRV »ý¼º
	result = device->CreateShaderResourceView(renderTargetTexture, &srvDesc, &shaderResourceView);
	...
	//ºä / Åõ¿µ Çà·Ä ¸¸µé±â
	view = XMMatrixLookAtLH(camera->GetPositionVector(), camera->GetCameraLook(), camera->GetCameraUp());
