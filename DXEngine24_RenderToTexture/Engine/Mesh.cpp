...
bool Mesh::CreateRasterizerState(ID3D11Device * device)
{
	...
	//오류 확인
	if (IsError(result, TEXT("래스터라이저 생성 실패")))
		return false;
...
void Mesh::BindeShaders(ID3D11DeviceContext * deviceContext)
{
...
{
	return material->AddTexture(fileName);
}
...
void Mesh::BindeTextures(ID3D11DeviceContext * deviceContext)
{
