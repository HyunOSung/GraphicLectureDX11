...
bool DefferedRenderer::Initialize(ID3D11Device * device, ID3DBlob * vertexShaderBuffer, Camera * camera, UINT screenWidth, UINT screenHeight)
{
	//정점 만들기
	...
	vertices[0].textureCoord = XMFLOAT2(1.0f, 0.0f);
	vertices[1].position = XMFLOAT3(right, bottom, 0.0f);
	vertices[1].textureCoord = XMFLOAT2(1.0f, 1.0f);
	vertices[2].position = XMFLOAT3(left, bottom, 0.0f);
	vertices[2].textureCoord = XMFLOAT2(0.0f, 1.0f);
	vertices[3].position = XMFLOAT3(left, top, 0.0f);
	vertices[3].textureCoord = XMFLOAT2(0.0f, 0.0f);
...
void DefferedRenderer::Render(ID3D11DeviceContext * deviceContext)
{
	UINT stride = sizeof(VertexType);
	UINT offset = 0;

	deviceContext->IASetInputLayout(inputLayout);
	deviceContext->IAGetVertexBuffers(0, 1, &vertexBuffer, &stride, &offset);
...
void DefferedRenderer::Update(ID3D11DeviceContext * deviceContext)
{
